...
function renderUI() {
  ...
  switch (mode) {
    ...
    case "splash":
      ...
      break;
    case "menu":
      ui.innerHTML = `<div class="menu"><header>${smallBrand}<span class="edition">ПРОСПЕКТ, ГЛАВПОЧТАМТ<br>ПЕРВАЯ ДОСТАВКА</span></header><div class="menu-copy"><div class="eyebrow"><i></i> ГОРОДСКАЯ ПОЧТА</div><h1>СПЕЦ<br>ДОСТАВКА<span>®</span></h1><p class="tagline">Будущее нуждается в правках.</p><p class="intro-copy">В Малом зале готовятся к концерту.<br>Доставьте расписание, которое остановит шумную линию.</p><nav>${button(save && !save.completed ? "continue" : "start", save ? (save.completed ? "Повторить доставку" : "Продолжить доставку") : "Начать доставку", "primary large")}<div class="menu-secondary">${button("city", "О городе", "text-button")}${button("settings", "Настройки", "text-button")}${button("about", "Об игре", "text-button")}</div></nav><div class="mission-mini"><span>12-Б</span><div>ПЕРВАЯ ДОСТАВКА<strong>«Перерыв»</strong></div><em>Маршрут<br>3–5 мин</em></div></div><div class="poster" aria-hidden="true"><div class="poster-circle"></div><div class="poster-orbit"></div><img src="${scene.menuBagURL || "/assets/parcel.png"}" alt=""><span class="poster-note">ОТПРАВЛЕНИЕ В МАЛЫЙ ЗАЛ</span><span class="poster-number">12-Б</span></div><footer><span>ЛЮДИ НЕ ДОЛЖНЫ ТЕРЯТЬ СВЯЗЬ</span><span>КОМПЬЮТЕР · КЛАВИАТУРА · ЗВУК</span></footer></div>`;
